...
So with a correlation of just under 0.5, we are looking at a moderately strong correlation. I'm going to conclude that my theory holds!
...
harris_choropleth_data <- harris_choropleth_data %>% 
  ...
  select(region, value)
zip_choropleth(harris_choropleth_data, county_zoom = 48201)
```

```{r}
harris_choropleth_data <- zip_data_sublist #create a new dataframe for this test
harris_choropleth_data$zip_code <- as.character(harris_choropleth_data$zip_code)
harris_choropleth_data <- harris_choropleth_data %>% 
