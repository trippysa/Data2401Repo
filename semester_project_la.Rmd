...
#for the same reason
la_masterlist <- la_masterlist %>% rename(zip_code = location.zip_code) 
la_masterlist$zip_code <- as.double(la_masterlist$zip_code)
```
...
```{r}
la_masterlist$price[la_masterlist$price == "$"] <- 1
la_masterlist$price[la_masterlist$price == "$$"] <- 2
la_masterlist$price[la_masterlist$price == "$$$"] <- 3
la_masterlist$price[la_masterlist$price == "$$$$"] <- 4
la_masterlist$price <- as.double(la_masterlist$price)
```
...
```{r}
zip_data_sublist <- la_masterlist %>% 
  group_by(Zip) %>% 
  ...
  right_join(la_data_by_zip, by = "Zip")
...
la_choropleth_data$zip_code <- as.character(la_choropleth_data$Zip)
...
la_choropleth_data <- la_choropleth_data %>% 
  rename(region = Zip, value = Median) %>% 
  select(region, value)
...
la_choropleth_data$Zip <- as.character(la_choropleth_data$Zip)
la_choropleth_data <- la_choropleth_data %>% 
  rename(region = Zip, value = restaurant_price) %>% 
  select(region, value)
zip_choropleth(la_choropleth_data, 
               county_zoom = 06037,
